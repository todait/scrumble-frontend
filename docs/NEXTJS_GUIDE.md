...
graph TD
    A[랜딩 페이지 /] --> B{로그인 상태?}
    B -->|No| C[/auth 리다이렉트]
    B -->|Yes| D[/workspace/create 리다이렉트]
    ...
    G --> H[토큰 저장]
    H --> I[사용자 정보 저장]
    I --> J[/workspace/create?auth=success]
    J --> K[성공 토스트 표시]
```
...
└── children
    ...
    └── app/workspace/create/page.tsx (워크스페이스 생성)
```
...
### 3.5 워크스페이스 생성 페이지 (`app/workspace/create/page.tsx`)
...
```typescript
// features/workspace/services/workspace.service.ts
