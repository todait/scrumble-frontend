...
**엔드포인트**: `POST /api/v1/notifications/bulk-read`

**설명**: 여러 개의 알림을 한 번에 읽음 처리합니다. 최대 100개까지 지원합니다.

**헤더**:

- `Authorization` (string, required): Bearer {space_member_jwt_token}
...
**엔드포인트**: `POST /api/v1/notifications/mark-all-read`
...
**설명**: 현재 로그인한 스페이스 멤버의 알림을 조회합니다. 페이지네이션, 카테고리 필터, 타입 필터, 읽음 상태 필터를 지원합니다.
...
**엔드포인트**: `GET /api/v1/notifications/unreadCount`

**설명**: 현재 로그인한 스페이스 멤버의 읽지 않은 알림 개수를 카테고리별로 조회합니다.
...
```json
{
  "error": {
    ...
    "details": {
      ...
      "constraint": "must contain 1-100 valid UUIDs"
    }
  },
  "meta": {
...
{
  "error": {
    "code": "SPACE_MEMBER_NO_PERMISSION",
    "message": "Forbidden",
    "details": {
      "reason": "Access to other user's notifications denied"
    }
  },
  "meta": {
    "request_id": "req-123456",
...
```json
{
  "error": {
    ...
    "message": "Not found",
    "details": {
      "resource": "Space member not found"
    }
  },
  "meta": {
    "request_id": "req-123456",
...
- SpaceMember JWT 토큰을 통해 인증된 사용자만 알림에 접근 가능
- 사용자는 자신이 로그인한 스페이스의 알림만 조회하고 읽음 처리할 수 있음
...
```bash
# 1. 읽지 않은 피드 알림 조회
GET /api/v1/notifications?categories=feed&is_read=false&limit=10
Headers:
  Authorization: Bearer {space_member_jwt_token}
...
# 2. 다음 페이지 조회
GET /api/v1/notifications?cursor=eyJjcmVhdGVkX2F0IjoiMjAyNC0wMS0xNVQxMDozMDowMFoiLCJpZCI6IjU1MGU4NDAwLWUyOWItNDFkNC1hNzE2LTQ0NjY1NTQ0MDAwMSJ9&limit=20
Headers:
  Authorization: Bearer {space_member_jwt_token}
```
...
GET /api/v1/notifications?types=comment,emoji_reaction&is_read=false&limit=30
Headers:
  Authorization: Bearer {space_member_jwt_token}

# 활동 카테고리의 모든 알림 조회
GET /api/v1/notifications?categories=activity
Headers:
  Authorization: Bearer {space_member_jwt_token}
```
...
- 🔐 **[FIX]** 인증 구조 정정: SpaceMember JWT 토큰은 Authorization 헤더로만 전송 (Cookie 사용 안함)
- 📝 **[FIX]** 에러 응답의 timestamp 필드가 숫자 타입으로 정정
- 🆕 **[REMOVE]** x-space-slug 헤더 제거 (JWT에서 자동 추출)

### v1.2.0 (2025-07-27)

- 🔐 **[BREAKING]** SpaceMember 인증 구조 도입
- 🔗 **[BREAKING]** API 경로 변경: 모든 알림 API는 `/api/v1/notifications` 하위로 통합
- 🆕 **[UPDATE]** spaceMemberId와 spaceSlug는 SpaceMember JWT 토큰에서 자동으로 추출 (별도 전달 불필요)
