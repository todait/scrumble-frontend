...
**참고**: SpaceMember 토큰에 이미 스페이스 정보가 포함되어 있어 URL에 spaceSlug가 필요하지 않습니다.
...
**엔드포인트**: `GET /api/v1/todos?date={date}`
...
- `spaceMemberID` (string, optional): 조회할 스페이스 멤버 ID (UUID 형식). 예: `123e4567-e89b-12d3-a456-426614174000`. 미제공 시 현재 SpaceMember의 Todo 조회

**헤더**:

- `Authorization` (string, required): Bearer {user_access_token}
- `Cookie` (string, required): scrumble*token*{spaceSlug}\_access={space_member_token}
...
**설명**: 여러 할 일을 한 번에 일괄 업데이트합니다. 효율적인 diff 알고리즘을 사용하여 생성/수정/삭제를 단일 트랜잭션으로 처리합니다. 이 엔드포인트는 새로운 할 일 생성과 기존 할 일 업데이트를 모두 처리합니다.
...
**엔드포인트**: `PATCH /api/v1/todos/{todoId}`
...
**엔드포인트**: `PATCH /api/v1/todos/{todoId}/toggle`
...
**엔드포인트**: `DELETE /api/v1/todos/{todoId}`
...
Headers:
  ...
  Cookie: scrumble_token_my-team_access={space_member_token}
  X-Timezone: Asia/Seoul
...
GET /api/v1/todos?date=2024-03-05
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
  X-Timezone: Asia/Seoul
...
# 2. 특정 할 일 완료 처리
PATCH /api/v1/todos/123e4567-e89b-12d3-a456-426614174000/toggle
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
```
...
```bash
# 할 일의 이름, 설명, 날짜 수정
PATCH /api/v1/todos/123e4567-e89b-12d3-a456-426614174000
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
  X-Timezone: Asia/Seoul
...
GET /api/v1/todos?date=2024-03-05&spaceMemberID=456e7890-e89b-12d3-a456-426614174001
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
  X-Timezone: Asia/Seoul

# 본인의 Todo 목록 조회 (spaceMemberID 파라미터 생략)
GET /api/v1/todos?date=2024-03-05
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
...
GET /api/v1/todos?date=2024-03-01
Headers:
  Authorization: Bearer {user_access_token}
  Cookie: scrumble_token_my-team_access={space_member_token}
  X-Timezone: Asia/Seoul
...
- 🔧 **API 구조 정리**: 실제 구현과 일치하도록 문서 수정
  - 할 일 생성은 일괄 업데이트 API(`POST /api/v1/todos`)로 통합
  - 중복된 섹션 제거 및 순서 재정렬
- 📝 **시나리오 업데이트**: 일괄 업데이트 API를 사용한 생성 예제로 수정
