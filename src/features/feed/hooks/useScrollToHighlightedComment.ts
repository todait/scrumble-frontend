...
export function useScrollToHighlightedComment({
  ...
  useEffect(() => {
    ...
    const waitForContentToLoad = async (container: HTMLElement): Promise<boolean> => {
      ...
      // 2. 투두리스트 로딩 확인 (로딩 스피너가 없고, 투두 컨테이너가 있는지)
      const hasLoadingSpinner = container.querySelector('.animate-spin');
      const todoSection = container.querySelector('[class*="CollapseSection"]');
      
      // 투두 섹션이 있고 로딩 중이 아닌 경우 OK
      // 투두 섹션이 없는 경우도 OK (투두가 없는 포스트)
