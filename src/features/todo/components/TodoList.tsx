...
export function TodoList({
  ...
  const addTodoFromInput = useCallback(
    (text: string, continueInserting = false) => {
      ...
      // 중간 삽입 상태 관리
      if (continueInserting && insertPosition !== null) {
        // Shift+Enter: 새로 추가된 Todo 아래에 계속 삽입
  ...
  const handleShiftSelectRange = useCallback(
    ...
    [onSelectionChange, disabledIds, sortedTodos, lastSelectedIndex, selectedIds, setLastSelectedIndex]
  );
  ...
  return (
    <div className="space-y-1">
      ...
      {/* 하단 고정 TodoInput (중간 삽입이 아닌 경우에만) */}
      {mode === 'edit' && insertPosition === null && (
        <TodoInput
          onAddTodo={handleInputAddTodo}
          isFocused={isInputFocused}
          onFocus={handleInputFocus}
